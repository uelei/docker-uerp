...
RUN cd /var/www/uerp/web/ && sed -i'' '7,13 s/^/#/' config.php && echo 'date.timezone = "America/Sao_Paulo"' > /etc/php5/apache2/php.ini && sed -i'' '13,13 s/^/#/' /var/www/uerp/app/config/parameters.yml &&echo '    locale: br ' > /var/www/uerp/app/config/parameters.yml
EXPOSE 80 22
